...
<style>
  ...
  .app-header .logo{height:36px} .app-header{ display:flex; align-items:center; gap:12px; padding:12px 0; color:#111; }
      ...
      <form method="POST" action="{{ route('registrasi-teknisi.update',$row) }}" class="card-like" enctype="multipart/form-data">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            @if($row->foto_path)
              <div class="mt-2">
                <img src="{{ Storage::url($row->foto_path) }}" alt="foto" style="height:56px;width:56px;border-radius:50%;object-fit:cover;border:1px solid #e5e7eb">
              </div>
            @endif
          </div>
        </div>
